google_peak_sh <- function(year = 6, plot = FALSE) {
    source("~/predictiveScience/R_Functions/google_flu_trends/find_google_peak_week.R")
    ...
    if (plot) {
        par(pty = "s")
        plot(x = b$peak_date, y = b$sh, ylab = "Specific Humidity", xlab = "Date of Peak Week", col = b$color, pch = 20)
