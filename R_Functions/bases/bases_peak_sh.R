bases_peak_sh <- function(year = 10, plot = FALSE) {
    ...
    dmss_years <- c("2000-2001", "2001-2002", "2002-2003", "2003-2004", "2004-2005", "2005-2006", "2006-2007", "2007-2008", "2008-2009", "2009-2010", "2010-2011")
    ...
    for (zz in year) {
        ...
        a <- data.frame(zip = as.integer(colnames(peak_week_bases)), peak = as.Date(unname(unlist(peak_week_bases[zz, ]))), sh = NA, state = NA, color = NA, stringsAsFactors = FALSE)
        ...
        for (i in 1:nrow(a)) {
            ...
            if (is.na(humidity)) {
                ...
                if (zip$State[index] == "HI" | zip$State[index] == "AK" | zip$State[index] == "AE" | zip$State[index] == "AP") {
                  remove_rows <- append(remove_rows, i)
        ...
        if (plot) {
            par(pty = "s")
            plot(x = a$peak, y = a$sh, xlab = "Date of Peak Week", ylab = "Average Specific Humidity Week Prior to Peak", col = a$color, pch = 20, ylim = c(0, 0.02))
            ...
            title <- paste("MPZ-DATA Peak", dmss_years[zz], "Against Specific Humidity\nBlack:", round(black_corr$r[2], 2), ", p ~", signif(black_corr$P[2], 1), "Red:", 
                round(red_corr$r[2], 2), ", p ~", signif(red_corr$P[2], 1))
            title(main = title)
        }
        return(a)
    }
