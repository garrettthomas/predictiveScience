bases_peak_distance <- function(year = 10, plot = FALSE) {
    ...
    for (zz in year) {
        ...
        a <- cbind(a, data.frame(lat = NA, long = NA, state = as.character(NA), color = as.character(NA), stringsAsFactors = FALSE))
        ...
        for (i in 1:nrow(b)) {
            ...
            for (j in 1:length(c)) {
                temp_data_frame[j, 1] <- gdist(lat.1 = as.numeric(b[j, 3]), lon.1 = as.numeric(b[j, 4]), lat.2 = as.numeric(q[1]), lon.2 = as.numeric(q[2]), units = "km")
            }
