bases_peak_school <- function(year = 10, plot = FALSE) {
    ...
    for (zz in year) {
        b <- data.frame(zip = as.integer(names(a)), peak = as.Date(unlist(unname(a[zz, ]))), state = as.character(NA), school_start = as.Date(NA), color = as.character(NA), 
            stringsAsFactors = FALSE)
        if (length(which(is.na(b$peak))) != 0) {
            b <- b[-1 * which(is.na(b$peak)), ]
        }
        # a[, c(3:5)] <- NA names(a)[c(3:5)] <- c('long', 'lat', 'state') location_data_frame <- read.csv('~/Dropbox/LEPR/data/AFHSC-top300-zip5.csv')[, c(4, 8, 9)]
        ...
        if (plot) {
            ...
            title <- paste("Peak Date of 2009 - 2010 Influenza Wave Against School Start Date\nBlack:", round(corr_b$r[2], 2), ", p ~", signif(corr_b$P[2], 1), "   Red:", 
                round(corr_r$r[2], 2), ", p ~", signif(corr_r$P[2], 1))
            title(main = title)
