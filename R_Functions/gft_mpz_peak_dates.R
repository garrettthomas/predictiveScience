...
for ( i in 1:nrow(a) ) {
  distance <- gdist(lon.1 = as.numeric(a[i, 4]), lat.1 = as.numeric(a[i, 3]), 
                            lon.2 = as.numeric(b[1, 9]), lat.2 = as.numeric(b[1, 8]), units = "km")
  distance_peaks[i,2] <- b[1,2]
  distance_peaks[i,3] <- distance
  distance_peaks[i,6] <- b[1,1]
  
  for ( j in 2:nrow(b) ) {
    dist <- gdist(lon.1 = as.numeric(a[i, 4]), lat.1 = as.numeric(a[i, 3]), 
            lon.2 = as.numeric(b[j,9]), lat.2 = as.numeric(b[j,8]), units = "km")
    if ( dist < distance ) {
