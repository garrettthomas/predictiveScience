...
#   }
# }
# date_week_num$week_start_date <- as.character(date_week_num$week_start_date)
# date_week_num$week_end_date <- as.character(date_week_num$week_end_date)
# 
...
###### LOAD GOOGLE FLU TRENDS CITY DATA, STANDARDIZE DATE FORMAT, CONVERT VALUES FROM CASES PER 100,000 PHYSICIAN VISITS
###### TO ILI PERCENT (DIVIDE BY 100,000), SET CONTRAINTS ON FIELDS (UNIQUE, PRIMARY KEY, ETC.)
...
for ( i in 2:ncol(GFT_City_Data) ) {
  GFT_City_Data[,i] <- GFT_City_Data[,i] / 100000
  
  
}

sql_gft_city_statement <- paste("create table GFT_CITY(Date text unique,", paste(names(GFT_City_Data[,c(2:ncol(GFT_City_Data))]), collapse = " integer, "), 
...
###### LOAD GOOGLE FLU TRENDS STATE DATA, STANDARDIZE DATE FORMAT, CONVERT VALUES FROM CASES PER 100,000 PHYSICIAN VISITS
###### TO ILI PERCENT (DIVIDE BY 100,000), SET CONTRAINTS ON FIELDS (UNIQUE, PRIMARY KEY, ETC.)
...
for ( i in 2:ncol(GFT_State_Data) ) {
  GFT_State_Data[,i] <- GFT_State_Data[,i] / 100000
}

sql_gft_state_statement <- paste("create table GFT_STATE(Date text unique,", paste(names(GFT_State_Data[,c(2:ncol(GFT_State_Data))]), collapse = " integer, "), 
                       "integer, primary key (Date))" )
...
###### LOAD GOOGLE FLU TRENDS HHS REGION DATA, STANDARDIZE DATE FORMAT, CONVERT VALUES FROM CASES PER 100,000 PHYSICIAN VISITS
###### TO ILI PERCENT (DIVIDE BY 100,000), SET CONTRAINTS ON FIELDS (UNIQUE, PRIMARY KEY, ETC.)
...
for ( i in 2:ncol(GFT_HHS_Regional_Data) ) {
  GFT_HHS_Regional_Data[,i] <- GFT_HHS_Regional_Data[,i] / 100000
}
...
####### LOAD CDC (ILINET) CENSUS REGION DATA, STANDARDIZE DATE FORMAT, DATA IS INHARIENTLY THREE DIMENSIONAL 
####### (DATE by REGION by VARIABLE. VARIABLES = ILITOTAL,TOTAL PATIENTS,NUM. OF PROVIDERS,% WEIGHTED ILI,%UNWEIGHTED ILI,
####### AGE 0-4,AGE 5-24,AGE 25-64,AGE 25-49,AGE 50-64,AGE 65 AND REGIONS ARE CENSUS REGIONS = 
####### NEW ENGLAND, MID-ATLANTIC, EAST NORTH CENTRAL, WEST NORTH CENTRAL, SOUTH ATLANTIC, EAST SOUTH CENTRAL, WEST SOUTH CENTRAL, 
####### MOUTAIN, PACIFIC), CONVERT SINGLE DATA FRAME TO ELEVEN DATA FRAMES
####### TO SIMULATE THREE DIMENSIONAL DATA (I'M NOT 100 PERCENT SURE THIS IS THE BEST WAY TO GO ABOUT THIS)



CDC_CENSUS_Regional_Data <- read.csv(file = "~/GFTvsILINET/DATA/FluViewPhase2Data CITY/ILINetCITY.csv", header = TRUE,
                                   check.names = TRUE, stringsAsFactors = FALSE)
...
for ( i in 1:length(CDC_CENSUS_LIST) ) {
  ...
  dbWriteTable(conn = BSVE_dbConn, name = paste("CDC_CENSUS_", names(CDC_CENSUS_Regional_Data)[i + 4], sep = ""), value = CDC_CENSUS_LIST[[i]], 
               row.names = FALSE)
...
####### LOAD CDC (ILINET) HHS REGION DATA, STANDARDIZE DATE FORMAT, DATA IS INHARIENTLY THREE DIMENSIONAL 
####### (DATE by REGION by VARIABLE. VARIABLES = ILITOTAL,TOTAL PATIENTS,NUM. OF PROVIDERS,% WEIGHTED ILI,%UNWEIGHTED ILI,
####### AGE 0-4,AGE 5-24,AGE 25-64,AGE 25-49,AGE 50-64,AGE 65), CONVERT SINGLE DATA FRAME TO ELEVEN DATA FRAMES
####### TO SIMULATE THREE DIMENSIONAL DATA (I'M NOT 100 PERCENT SURE THIS IS THE BEST WAY TO GO ABOUT THIS)



CDC_HHS_Regional_Data <- read.csv(file = "~/GFTvsILINET/DATA/FluViewPhase2Data Regional/ILINetRegional.csv", header = TRUE,
                                  check.names = TRUE, stringsAsFactors = FALSE)
...
CDC_HHS_LIST[[i-4]] <- data.frame(Year = CDC_HHS_Regional_Data$YEAR[seq(1, (nrow(CDC_HHS_Regional_Data) - 9), by = 10)], 
                        WEEK = CDC_HHS_Regional_Data$WEEK[seq(1, (nrow(CDC_HHS_Regional_Data) - 9), by = 10)],
                        HHS_REGION_1 = CDC_HHS_Regional_Data[seq(1, (nrow(CDC_HHS_Regional_Data) - 9), by = 10), i],
                        HHS_REGION_2 = CDC_HHS_Regional_Data[seq(2, (nrow(CDC_HHS_Regional_Data) - 8), by = 10), i],
                        HHS_REGION_3 = CDC_HHS_Regional_Data[seq(3, (nrow(CDC_HHS_Regional_Data) - 7), by = 10), i],
                        HHS_REGION_4 = CDC_HHS_Regional_Data[seq(4, (nrow(CDC_HHS_Regional_Data) - 6), by = 10), i],
                        HHS_REGION_5 = CDC_HHS_Regional_Data[seq(5, (nrow(CDC_HHS_Regional_Data) - 5), by = 10), i],
                        HHS_REGION_6 = CDC_HHS_Regional_Data[seq(6, (nrow(CDC_HHS_Regional_Data) - 4), by = 10), i],
                        HHS_REGION_7 = CDC_HHS_Regional_Data[seq(7, (nrow(CDC_HHS_Regional_Data) - 3), by = 10), i],
                        HHS_REGION_8 = CDC_HHS_Regional_Data[seq(8, (nrow(CDC_HHS_Regional_Data) - 2), by = 10), i],
                        HHS_REGION_9 = CDC_HHS_Regional_Data[seq(9, (nrow(CDC_HHS_Regional_Data) - 1), by = 10), i],
                        HHS_REGION_10 = CDC_HHS_Regional_Data[seq(10, (nrow(CDC_HHS_Regional_Data)), by = 10), i])
}
...
####### LOAD CDC (ILINET) NATIONAL DATA, STANDARDIZE DATE FORMAT, DATA IS TWO DIMENSIONAL 
####### BECAUSE THERE IS ONLY ONE REGION (NATIONAL) 


CDC_National_Data <- read.csv(file = "~/GFTvsILINET/DATA/FluViewPhase2DataNational/ILINet_national.csv", 
...
####### LOAD WHO (NREVSS) CENSUS REGION DATA, STANDARDIZE DATE FORMAT, DATA IS INHARIENTLY THREE DIMENSIONAL 
####### (DATE by REGION by VARIABLE. VARIABLES = TOTAL SPECIMENS,PERCENT POSITIVE,A (H1),A (Unable to Subtype),
####### A (H3),2009 H1N1,A (Subtyping not Performed),B,H3N2v AND REGIONS ARE CENSUS REGIONS = 
####### NEW ENGLAND, MID-ATLANTIC, EAST NORTH CENTRAL, WEST NORTH CENTRAL, SOUTH ATLANTIC, EAST SOUTH CENTRAL, WEST SOUTH CENTRAL, 
####### MOUTAIN, PACIFIC), CONVERT SINGLE DATA FRAME TO NINE DATA FRAMES
####### TO SIMULATE THREE DIMENSIONAL DATA (I'M NOT 100 PERCENT SURE THIS IS THE BEST WAY TO GO ABOUT THIS)



WHO_CENSUS_Regional_Data <- read.csv(file = "~/GFTvsILINET/DATA/FluViewPhase2Data CITY/WHO_NREVSSCITY.csv", header = TRUE,
                                   check.names = TRUE, stringsAsFactors = FALSE)
...
for ( i in 1:length(WHO_CENSUS_LIST) ) {
  ...
  dbWriteTable(conn = BSVE_dbConn, name = paste("WHO_CENSUS_", names(WHO_CENSUS_Regional_Data)[i + 4], sep = ""), value = WHO_CENSUS_LIST[[i]], 
               row.names = FALSE)
...
####### LOAD WHO (NREVSS) HHS REGION DATA, STANDARDIZE DATE FORMAT, DATA IS INHARIENTLY THREE DIMENSIONAL 
####### (DATE by REGION by VARIABLE. VARIABLES = TOTAL SPECIMENS,PERCENT POSITIVE,A (H1),A (Unable to Subtype),
####### A (H3),2009 H1N1,A (Subtyping not Performed),B,H3N2v), CONVERT SINGLE DATA FRAME TO NINE DATA FRAMES
####### TO SIMULATE THREE DIMENSIONAL DATA (I'M NOT 100 PERCENT SURE THIS IS THE BEST WAY TO GO ABOUT THIS)



WHO_HHS_Regional_Data <- read.csv(file = "~/GFTvsILINET/DATA/FluViewPhase2Data Regional/WHO_NREVSS_REGIONAL.csv", header = TRUE,
                                  check.names = TRUE, stringsAsFactors = FALSE)
...
for ( i in 5:ncol(WHO_HHS_Regional_Data)) {
  WHO_HHS_LIST[[i-4]] <- data.frame(Year = WHO_HHS_Regional_Data$YEAR[seq(1, (nrow(WHO_HHS_Regional_Data) - 9), by = 10)], 
                                    WEEK = WHO_HHS_Regional_Data$WEEK[seq(1, (nrow(WHO_HHS_Regional_Data) - 9), by = 10)],
                                    HHS_REGION_1 = WHO_HHS_Regional_Data[seq(1, (nrow(WHO_HHS_Regional_Data) - 9), by = 10), i],
                                    HHS_REGION_2 = WHO_HHS_Regional_Data[seq(2, (nrow(WHO_HHS_Regional_Data) - 8), by = 10), i],
                                    HHS_REGION_3 = WHO_HHS_Regional_Data[seq(3, (nrow(WHO_HHS_Regional_Data) - 7), by = 10), i],
                                    HHS_REGION_4 = WHO_HHS_Regional_Data[seq(4, (nrow(WHO_HHS_Regional_Data) - 6), by = 10), i],
                                    HHS_REGION_5 = WHO_HHS_Regional_Data[seq(5, (nrow(WHO_HHS_Regional_Data) - 5), by = 10), i],
                                    HHS_REGION_6 = WHO_HHS_Regional_Data[seq(6, (nrow(WHO_HHS_Regional_Data) - 4), by = 10), i],
                                    HHS_REGION_7 = WHO_HHS_Regional_Data[seq(7, (nrow(WHO_HHS_Regional_Data) - 3), by = 10), i],
                                    HHS_REGION_8 = WHO_HHS_Regional_Data[seq(8, (nrow(WHO_HHS_Regional_Data) - 2), by = 10), i],
                                    HHS_REGION_9 = WHO_HHS_Regional_Data[seq(9, (nrow(WHO_HHS_Regional_Data) - 1), by = 10), i],
                                    HHS_REGION_10 = WHO_HHS_Regional_Data[seq(10, (nrow(WHO_HHS_Regional_Data)), by = 10), i])
}
...
####### LOAD WHO (NREVSS) NATIONAL DATA, STANDARDIZE DATE FORMAT, DATA IS TWO DIMENSIONAL 
####### BECAUSE THERE IS ONLY ONE REGION (NATIONAL) 


WHO_National_Data <- read.csv(file = "~/GFTvsILINET/DATA/FluViewPhase2DataNational/ILINet_national.csv", 
                              header = TRUE, check.names = TRUE, stringsAsFactors = FALSE)
